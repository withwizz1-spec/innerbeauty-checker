...
import { CATEGORY_COLOR } from '../data/ingredientCategory'
    ...
    <div>
      <div className="ingredient-section-head">
        ...
        <button
          ...
          className="btn-plain"
          onClick={() => setOpen((v) => !v)}
          style={{ color: 'var(--brand-strong)', fontSize: '0.78rem', textDecoration: 'underline', flexShrink: 0 }}
        >
          ⓘ 기능성원료·첨가물·미확인이 뭔가요?
        </button>
      </div>
      ...
      {open && (
        ...
        >
          <div>
            <strong style={{ color: CATEGORY_COLOR.functional }}>기능성원료</strong> — 이 제품이 "건강기능식품"으로
            인정받게 만든 핵심 성분이에요. 식약처가 특정 건강 기능(예: 면역기능, 항산화 등)에 대해
            효과와 안전성을 직접 심사해서 인정한 성분입니다.
          </div>
          <div>
            <strong style={{ color: CATEGORY_COLOR.additive }}>첨가물</strong> — 정제·캡슐 형태를 만들거나, 맛을
